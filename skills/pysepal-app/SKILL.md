...
- Always pass `SepalMap(gee=False)` or `SepalMap(gee_interface=get_current_gee_interface())` in a container app: the bare `gee=True` default calls `su.init_ee()` and now raises `SepalSessionError` in a per-connection runtime.
