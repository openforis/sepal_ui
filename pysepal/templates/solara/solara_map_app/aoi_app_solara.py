    ...
    with solara.Column(style="padding: 20px; gap: 20px;"):
        ...
        with solara.Card("Notification Demo", elevation=1):
            ...
            with solara.Row(style="gap: 8px; flex-wrap: wrap;"):
                solara.Button(
                    "Success",
                    on_click=lambda: notify("Operation completed successfully!", type_="success"),
                    color="success",
                    outlined=True,
                )
                solara.Button(
                    "Info",
                    on_click=lambda: notify("Here is some useful information.", type_="info"),
                    color="info",
                    outlined=True,
                )
                solara.Button(
                    "Warning",
                    on_click=lambda: notify("Please check your configuration.", type_="warning"),
                    color="warning",
                    outlined=True,
                )
                solara.Button(
                    "Error",
                    on_click=lambda: notify("Something went wrong!", type_="error"),
                    color="error",
                    outlined=True,
                )
