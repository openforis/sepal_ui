...
//   2. config_locale prop                — one-time legacy ~/.sepal-ui-config
//                                          seed (copied to localStorage so
//                                          the migration happens once)
//   3. navigator.language               — browser auto-detection
//   4. "en"
//
...
// silently dropped when this widget is a root widget.
//
...
export default {
  ...
  props: {
    ...
    config_locale: { type: String, default: "" },
  },
  ...
  mounted() {
    ...
    const seeded = this.matchOffered(this.config_locale, offered);
    if (seeded) {
      // one-time migration: the legacy config choice becomes a browser pick
      this.storageSet(seeded);
      this.apply(seeded);
      return;
    }
