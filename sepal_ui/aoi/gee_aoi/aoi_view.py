...
from sepal_ui.aoi.gee_aoi.aoi_model import AoiModel
...
class AdminField(v.Select, sw.SepalWidget):
    ...
    def get_items(self, filter_=None):
        ...
        gaul_df = pd.read_csv(self.GAUL_FILE).drop_duplicates(subset=f'ADM{self.level}_CODE')
        
        # filter it 
        if filter_: gaul_df = gaul_df[gaul_df[f'ADM{self.level-1}_CODE'] == filter_]
        
        # formatted as a item list for a select component
        self.items = [
            {
                'text': su.normalize_str(r[f'ADM{self.level}_NAME'], folder=False), 
                'value': r[f'ADM{self.level}_CODE']
            } for _, r in gaul_df.iterrows()
        ] 
...
class AoiView(v.Card):
    ...
    def __init__(self, methods='ALL', map_=None, *args, **kwargs):
        ...
        self.w_asset = sw.AssetSelect(label=ms.aoi_sel.asset).hide()
    ...
    def _update_aoi(self, widget, event, data):
        """load the feature_collection in the model & update the map (if possible)"""
        
        # update the model 
        self.model.set_feature_collection()
        
        # update the map
        if self.map_:
            [self.map_.remove_layer(l) for l in self.map_.layers if l.name == 'aoi']
            self.map_.zoom_bounds(self.model.total_bounds())
            self.map_.addLayer(self.model.feature_collection, {'color':v.theme.themes.dark.success}, 'aoi')
            self.map_.hide_dc()
